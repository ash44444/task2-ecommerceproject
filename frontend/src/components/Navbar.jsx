...
export default function Navbar() {
  ...
  const toggleMobile = (e) => {
    // stopPropagation so parent elements/overlays don't intercept
    e?.stopPropagation?.();
    console.log("hamburger clicked, toggling mobile:", !mobileOpen);
    setMobileOpen((s) => !s);
  };
  ...
  const MobileLink = ({ to, children }) => (
    <Link
      to={to}
      onClick={closeMobile}
      className="block px-4 py-3 text-base text-gray-800 hover:bg-gray-100 rounded-md transition"
    >
      {children}
    </Link>
  );
  ...
  return (
    <nav className="sticky top-0 z-40 bg-white/90 backdrop-blur-md shadow-sm border-b border-gray-200">
      <div className="max-w-6xl mx-auto px-3 sm:px-4 lg:px-6 py-2.5 sm:py-3 flex items-center justify-between gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          <div className="hidden sm:block">
            {userName ? (
              <button onClick={handleLogout} className="text-[10px] sm:text-xs bg-red-600 text-white px-3 sm:px-4 py-1.5 rounded-full font-semibold shadow-sm hover:bg-red-700 transition" type="button">Logout</button>
            ) : (
              <button onClick={goToAuth} className="text-[10px] sm:text-xs bg-indigo-600 text-white px-3 sm:px-4 py-1.5 rounded-full font-semibold shadow-sm hover:bg-indigo-700 transition" type="button">Login</button>
            )}
          </div>
      ...
      {/* Overlay + Mobile Menu (no document listeners) */}
      {mobileOpen && (
        <>
          ...
          <div className="fixed top-16 left-4 right-4 z-50 mx-auto max-w-md">
            <div className="bg-white rounded-xl shadow-lg ring-1 ring-black/5 overflow-hidden">
              <div className="p-3">
                ...
                {role === "admin" && (
                  <Link to="/admin" onClick={closeMobile} className="block px-4 py-3 text-base text-red-600 font-semibold rounded-md hover:bg-red-50 transition">Admin</Link>
                )}
                ...
                <div className="mt-3 px-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div>
                      {userName ? (
                        <button onClick={() => { closeMobile(); handleLogout(); }} className="text-sm bg-red-600 text-white px-3 py-1 rounded-full font-semibold hover:bg-red-700 transition" type="button">Logout</button>
                      ) : (
                        <button onClick={() => { closeMobile(); goToAuth(); }} className="text-sm bg-indigo-600 text-white px-3 py-1 rounded-full font-semibold hover:bg-indigo-700 transition" type="button">Login</button>
                      )}
                    </div>
                  </div>
                </div>
